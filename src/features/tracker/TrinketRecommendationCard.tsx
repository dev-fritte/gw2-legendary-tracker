...
import { Sparkles, Users, ChevronDown, ChevronRight } from 'lucide-react';
import { ProfessionIcon } from '@/components/ProfessionIcon';
import { getProfessionMeta } from '@/utils/professions';
import { cn } from '@/utils/cn';
import type { LegendaryTrinketRecommendation } from '@/types/gw2-api';
import { DUAL_TRINKET_SLOT_TYPES } from '@/utils/trinketProperties';

interface TrinketRecommendationCardProps {
  recommendation: LegendaryTrinketRecommendation;
  rank: number;
  isCovered?: boolean;
  professionIcons: Map<string, string>;
}

export function TrinketRecommendationCard({
  recommendation,
  rank,
  isCovered = false,
  professionIcons,
}: TrinketRecommendationCardProps) {
  const { t } = useTranslation();
  const { slotType, impact, affectedCharacters, existingLegendaryCount, hasEquippedLegendary, icon } =
    recommendation;
  ...
  const slotLabel =
    slotType === 'Back'
      ? t('trinketTypes.Back')
      : t(`trinketTypes.${slotType}`, { defaultValue: slotType });
                ...
                <div className="flex flex-wrap gap-2">
                  {existingLegendaryCount > 0 && (
                    <p className="text-xs flex items-center gap-1" style={{ color: '#e9c66b' }}>
                      ...
                      {isDual && existingLegendaryCount < 2
                  ...
                  {isDual && !isCovered && (
                    <p className="text-xs" style={{ color: '#6a6478' }}>
                      {t('tracker.dualCoverageNote')}
                    </p>
                  )}
                </div>
              </div>
